...
/* Компонент */
	...
	function showHidenList(){
		var menu_sort = JSON.parse(localStorage.getItem('menu_sort')) || [];
		let menu_sort_obj = {};
		for (let i = 0; i < menu_sort.length; i++) {
			menu_sort_obj[menu_sort[i]] = menu_sort[i];
		}
		localStorage.setItem("menu_sort_obj", JSON.stringify(menu_sort_obj));
	}
	showHidenList();
	...
	Lampa.SettingsApi.addParam({
					...
					param: {
						...
						type: 'select',
						values: JSON.parse(localStorage.getItem("menu_sort_obj")),
					},
					field: {
						name: 'Скрывать пункт меню',
				    ...
				    onChange: function (value) {
						...
						$(".menu__item > .menu__text:contains('" + localStorage.getItem('list_hidden_items') + "')").parent().addClass('hide')
				    },
					onRender: function (item) {
						setTimeout(function() {
							document.querySelector("div > div.settings-param__value").text('Не выбран')
						}, 10);
					}
	});
