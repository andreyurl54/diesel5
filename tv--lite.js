...
if (!Lampa.Player.opened) Lampa.Player.opened = function(){return $('.player').is(':visible')};
...
function pluginPage(object) {
	...
	// var items = [];
	var html = $('<div class="' + plugin.component + '_main"></div>');
			...
			var parseList = function () {
				...
				while (i < l.length) {
					...
					if (!!channel.Url && !channel.isYouTube) {
						...
						channel['epgId'] = (channel['tvg-id'] && /^\d+$/.test(channel['tvg-id'])) ? channel['tvg-id'] : epgIdByName(channel['Title'], true);
						channel['Title'] = channel['Title'].replace(/\s+/g, ' ').trim();
						if (!channel['tvg-logo'] && channel['epgId'] && /^\d+$/.test(channel['epgId'])) {
	...
	function epgRender(epgId) {
		...
		if (!enableCardEpg) epgEl.css('visibility','hidden');
	...
	this.append = function (data) {
		...
		var bulkFn = bulkWrapper(function (channel) {
			...
			var cardAge = card.find('.card__age');
			if (!cardAge.length) {
			    cardAge = $('<div class="card__age"></div>');
			    card.append(cardAge);
		    }
			cardAge.html('<div class="card__epg-progress js-epgProgress"></div><div class="card__epg-title js-epgTitle">&nbsp;</div>')
			...
			card.on('hover:focus hover:hover touchstart', function (event) {
				...
				if (Lampa.PlayerPlaylist) {
				    Lampa.Keypad.listener.destroy();
				    if(Lampa.Keypad.listener._listeners === null) Lampa.Keypad.listener._listeners = {};
	...
	this.build = function (data) {
		...
		Lampa.Template.add(plugin.component + '_button_category', "<style>." + plugin.component + "_main .info{display:-webkit-box;display:-webkit-flex;display:-moz-box;display:-ms-flexbox;display:flex;height:10em;padding:2em 1.5em;-webkit-box-align:start;-webkit-align-items:flex-start;-moz-box-align:start;-ms-flex-align:start;align-items:flex-start;padding-top:1em}." + plugin.component + "_main .info__left{-webkit-box-flex:1;-webkit-flex-grow:1;-moz-box-flex:1;-ms-flex-positive:1;flex-grow:1;overflow:hidden}." + plugin.component + "_main .info__right{-webkit-flex-shrink:0;-ms-flex-negative:0;flex-shrink:0;display:-webkit-box;display:-webkit-flex;display:-moz-box;display:-ms-flexbox;display:flex;padding-top:1em}." + plugin.component + "_main .full-start__button{margin-right:.75em;font-size:1.3em;background-color:rgba(0,0,0,.3);padding:.3em 1em;display:-webkit-box;display:-webkit-flex;display:-moz-box;display:-ms-flexbox;display:flex;-webkit-border-radius:1em;-moz-border-radius:1em;border-radius:1em;-webkit-box-align:center;-webkit-align-items:center;-moz-box-align:center;-ms-flex-align:center;align-items:center;height:2.8em;-webkit-flex-shrink:0;-ms-flex-negative:0;flex-shrink:0}." + plugin.component + "_main .full-start__button>svg{width:1.5em;height:1.5em;-webkit-flex-shrink:0;-ms-flex-negative:0;flex-shrink:0}." + plugin.component + "_main .full-start__button>svg+span{margin-left:.75em;margin-top:-.2em}." + plugin.component + "_main .full-start__button.focus{background-color:#fff;color:#000}@media screen and (max-width: 2560px) {." + plugin.component + " .card--collection{width:14.3%!important;}}@media screen and (max-width: 800px) {." + plugin.component + " .card--collection {width: 24.6%!important;}}@media screen and (max-width: 500px) {." + plugin.component + " .card--collection {width: 33.3%!important;}}</style><div class=\"full-start__button selector view--category\"><svg style=\"enable-background:new 0 0 512 512;\" version=\"1.1\" viewBox=\"0 0 24 24\" xml:space=\"preserve\" xmlns=\"http://www.w3.org/2000/svg\" xmlns:xlink=\"http://www.w3.org/1999/xlink\"><g id=\"info\"/><g id=\"icons\"><g id=\"menu\"><path d=\"M20,10H4c-1.1,0-2,0.9-2,2c0,1.1,0.9,2,2,2h16c1.1,0,2-0.9,2-2C22,10.9,21.1,10,20,10z\" fill=\"currentColor\"/><path d=\"M4,8h12c1.1,0,2-0.9,2-2c0-1.1-0.9-2-2-2H4C2.9,4,2,4.9,2,6C2,7.1,2.9,8,4,8z\" fill=\"currentColor\"/><path d=\"M16,16H4c-1.1,0-2,0.9-2,2c0,1.1,0.9,2,2,2h12c1.1,0,2-0.9,2-2C18,16.9,17.1,16,16,16z\" fill=\"currentColor\"/></g></g></svg><span>" + langGet('categories') + "</span>\n	</div>");
...
if (!Lampa.Lang) {
	var lang_data = {'title_action':{'ru':'Действие'}};
	Lampa.Lang = {
		add: function add(data) {
			$.extend(lang_data, data);
		},
...
function addSettings(type, param) {
	...
	!!Lampa.SettingsApi && Lampa.SettingsApi.addParam(data);
}
...
function configurePlaylist(i) {
	...
	var name = getStorage('list_name_' + i, i ? '' : plugin.name);
	var url = getStorage('list_url_' + i, i ? '' : langGet('default_playlist'));
...
// Готовим настройки
!!Lampa.SettingsApi && Lampa.SettingsApi.addComponent(plugin);
addSettings(
	'trigger',
	{
