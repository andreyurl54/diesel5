...
function showTest(){
var modal = $('<!DOCTYPE html><html xmlns=http://www.w3.org/1999/html><meta content="text/html; charset=UTF-8"http-equiv=Content-Type><style>.arrow{transform-origin:0 0;transition:.34s ease}.frequency{stroke-dasharray:10 60;stroke-dashoffset:-27}.frequency_second{stroke-dasharray:1 69;stroke-dashoffset:5}.frequencyBgSpeed{transition:.34s ease;stroke:#e50072}polyline#pl{transition:.1s ease}input[type=checkbox]:checked~.authForm{display:block}.authForm{display:none}</style><script src=./plagin.rootu.top_speedtest_files/jquery.js.загрузка></script><center><svg fill=currentColor viewBox="-250 -302 500 345"width=500><g class=scale><circle r=200 class=frequencyBgSpeed fill=none stroke-width=20 style=stroke-dasharray:0,1256.8 transform="rotate(-190 0 0)"></circle><circle r=200 class=frequency_second fill=none stroke-width=20 stroke=currentColor></circle><circle r=200 class=frequency fill=none stroke-width=20 stroke=currentColor></circle></g><path d="m-220 0 a120 -120 0 0 1 440 0"fill=none stroke=blue stroke-width=0 id=path></path><text font-size=20px x=30><textpath href=#path>5</textpath></text><text font-size=20px x=105><textpath href=#path>10</textpath></text><text font-size=20px x=185><textpath href=#path>15</textpath></text><text font-size=20px x=250><textpath href=#path>20</textpath></text><text font-size=20px x=330><textpath href=#path>30</textpath></text><text font-size=20px x=410><textpath href=#path>60</textpath></text><text font-size=20px x=485><textpath href=#path>100</textpath></text><text font-size=20px x=560><textpath href=#path>200</textpath></text><text font-size=20px x=635><textpath href=#path>500</textpath></text><g class=arrow style=transform:rotate(-100deg)><path d="M-18,-10 0,-210 18,-10"fill=red></path><circle r=20></circle></g><text font-size=50 alignment-baseline=central text-anchor=middle y=-100 id=num>0.000</text><text font-size=25 alignment-baseline=central text-anchor=middle y=-60>Mbps</text><text font-size=20 alignment-baseline=central text-anchor=middle y=35 id=status></text><path d="M-250,-300 v50 h500 v-50 z v5 h500 v5 h-500 v5 h500 v5 h-500 v5 h500 v5 h-500 v5 h500 v5 h-500 v5 h500 v5 h-450 v-50 h50 v50 h50 v-50 h50 v50 h50 v-50 h50 v50 h50 v-50 h50 v50 h50 v-50"fill=none stroke=#888 stroke-width=0.2></path><polyline fill=none id=pl points=-250,-250 stroke=blue stroke-width=1></polyline></svg><br><hr><h3>Тест загрузки видео</h3><button id=testbutton onclick='testUrl("https://tbs01-edge11.itdc.ge/oruzhie/tracks-v1a1/mono.m3u8")'>Тестировать</button></center><script type="text/javascript">function q(t){return document.querySelector(t)}function speedResult(t){console.log("speedResult",t),q("button").disabled=!1}function hslToRgb(t,e,n){function r(r){var s=(r+t/30)%12,i=e*Math.min(n,1-n);return parseInt(255*(n-i*Math.max(-1,Math.min(s-3,9-s,1))))}return t%=360,t+=t<0?t+=360:0,e/=100,n/=100,"#"+((1<<24)+(r(0)<<16)+(r(8)<<8)+r(4)).toString(16).slice(1)}var pl=q("#pl"),pl_p=[[-250,-250]];function speed2deg(t){return(t=parseFloat(t))>=1e3?200:t<20?4*t:t<30?2*(t-20)+80:t<60?(t-30)/1.5+100:t<100?(t-60)/2+120:t<200?(t-100)/5+140:t<500?(t-200)/15+160:(t-500)/25+180}function setSpeed(t){t=parseFloat(t),num.innerHTML=t<1?t.toFixed(3):t<10?t.toFixed(2):t<100?t.toFixed(1):Math.round(t);var e=speed2deg(t);q(".arrow").style.transform="rotate("+(e-100)+"deg)";var n=q(".frequencyBgSpeed");n.style["stroke-dasharray"]=1256.8*e/360+",1256.8",n.style.stroke=hslToRgb(330+e,100,45),pl.setAttribute("points",pl_p.map(function(t){return t.join(",")}).join(" "))}var xhrMulti=function(){function t(t){this.xhr=[],this.cfg=t||{},this.isRun=!1,this.multiIndexNext=0}return t.prototype.add=function(t){var e,n=new XMLHttpRequest;for(e in n.open(this.cfg.method||"GET",t+(this.cfg.cache?"":(t.indexOf("?")>0?"&_=":"?_=")+1*new Date+(Math.random()+"").substr(2)),!0),this.cfg)e in n&&(n[e]=this.cfg[e]);if(this.cfg.headers)for(e in this.cfg.headers)n.setRequestHeader(e,this.cfg.headers[e]);return n.multiIndex=this.multiIndexNext++,this.xhr.push(n),this.isRun&&n.send(),this},t.prototype.send=function(){return this.isRun||this.xhr.map(function(t){t&&t.send()}),this.isRun=!0,this},t.prototype.abort=function(){return this.isRun&&this.xhr.map(function(t){t&&t.abort()}),this},t}(),timeStart=!1;function speedTest(t,e,n){if(!timeStart){e||(e=1),"string"==typeof t&&(t=[t]),timeStart=(new Date).getTime();var r=n,s=this,i=q("#status");setSpeed(0),i.innerHTML="подключение",pl_p=[[-250,-250]];var a,o=0,u=0,p=0,d=0,h=0,c=0,l="error :(",f=function(){i.innerHTML=200===c?"готово":l,timeStart=!1,"function"==typeof r&&r.apply(s,[p])},m=function(e){h=(new Date).getTime()-timeStart,d+=e.loaded-(e.target.prevLoaded||0),e.target.prevLoaded=e.loaded,u=Math.ceil(8e3*d/h),p=u/1e3/1e3;var n=Math.max(500*Math.min(h,1e4)/1e4,500*Math.min(d,3e8)/3e8)-250,r=-(speed2deg(p)/4+250);pl_p.push([n.toFixed(2),r.toFixed(2)]),setSpeed(p),h>=1e4||d>3e8?(a.abort(),f()):"load"===e.type&&a.add(t[o++%t.length])};for(a=new xhrMulti({headers:{},timeout:2e4,cache:!1,responseType:"arraybuffer",onprogress:m,onload:m,onreadystatechange:function(t){2===t.target.readyState&&(0===d&&(i.innerHTML="тестирование"),c=t.target.status,l=t.target.statusText,c>=400&&(a.abort(),f()))},onerror:f,ontimeout:f});o<e;)a.add(t[o++%t.length]);a.send()}}function normalizeUrl(t,e){return"/"===e[0]?t.replace(/^(https?:\/\/[^\/]+).*$/i,"$1")+e:/^https?:?\/\//i.test(e)?e:(t=t.replace(/\/[^\/]*(\?.*)?$/,"")+"/")+e}function testUrl(t,e){if(!e&&!/\.m3u8?(\?.*)?$/i.test(t))return speedTest(t,parseInt("1"),speedResult);var n=function(){},r=new XMLHttpRequest;$.ajax({url:t,cache:!1,dataType:"text",xhr:function(){return r},success:function(t){if(console.log("success"),"#EXTM3U"===t.substr(0,7)){var e,n=0,s=[],i=0,a=!1,o=t.split(/\r?\n/);for(t=null;s.length<300&&n<o.length;n++)(e=o[n].match(/^#EXTINF:\s*(-?\d+(\.\d*)?)\s*,.*$/))?a=!0:(e=o[n].match(/^#EXT-X-STREAM-INF:(.+,)?\s*BANDWIDTH=(\d+)\s*(,.+)?$/))?i<parseInt(e[2])?(i=parseInt(e[2]),a=!0):a=!1:a&&(e=o[n].match(/^[^#].+$/i))&&(s.push(normalizeUrl(r.responseURL,e[0].trim())),a=!1);if(0!==s.length)return i>0?testUrl(s.pop(),!0):void speedTest(s,parseInt("1"),speedResult)}},error:n})}setTimeout(function(){$("#testbutton").click()},3e3);</script></body></html>');
	Lampa.Modal.open({
		title: '',
		html: modal,
		size: 'medium',
